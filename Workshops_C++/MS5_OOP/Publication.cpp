...
namespace sdds
{
	ostream& Publication::prnInWidth(ostream& ostr, size_t width, const char* text)const
	{
		int i;
		for(i = 0; text[i] && i < width; i++) {
			ostr << text[i];
		}

		if (width == SDDS_TITLE_WIDTH)
		{
			for (; i < width; i++) {
				ostr << ".";
			}
	...
	ostream& Publication::write(ostream& os) const
	{
		...
		if (conIO(os))
		{
			...
			os.unsetf(ios::right);
			os.setf(ios::left);
			prnInWidth(os, SDDS_TITLE_WIDTH, m_title);
			os.unsetf(ios::left);
			os.setf(ios::right);
			os.fill(' ');
			if (m_membership != 0)
